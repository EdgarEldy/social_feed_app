...
/// The page of items returned by this call.
 List<T> get items;/// The cursor to pass on the next call to fetch the following page, or
...
bool operator ==(Object other) {
  final _this = this as PaginatedResult<T>;
  return identical(this, other) || (other.runtimeType == runtimeType&&other is PaginatedResult<T>&&const DeepCollectionEquality().equals(other.items, _this.items)&&(identical(other.nextCursor, _this.nextCursor) || other.nextCursor == _this.nextCursor));
}
...
String toString() {
  final _this = this as PaginatedResult<T>;
  return 'PaginatedResult<$T>(items: ${_this.items}, nextCursor: ${_this.nextCursor})';
}
...
class _PaginatedResult<T> implements PaginatedResult<T> {
  const _PaginatedResult({required  List<T> items, this.nextCursor}): _items = items;
...
@override
bool operator ==(Object other) {
    return identical(this, other) || (other.runtimeType == runtimeType&&other is _PaginatedResult<T>&&const DeepCollectionEquality().equals(other.items, _items)&&(identical(other.nextCursor, nextCursor) || other.nextCursor == nextCursor));
}
